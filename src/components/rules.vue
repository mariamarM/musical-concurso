...
const cardData: BentoCardProps[] = [
  {
    color: '#000C00',
    title: '',
    description: 'You can reproduce songs and guess them with a 5 second preview clip',
    label: 'How does it work?',
  },
  {
    color: '#000C00',
    title: '',
    description: 'Choose the correct song from the 4 possible options.',
    label: 'Guess the song',
  },
  {
    color: '#000C00',
    title: 'See your score and ranking',
    description: 'Every correct answer gives you points to climb the ranking',
    label: 'See your score and ranking',
  },
  {
    color: '#000C00',
    title: '',
    description: 'You can practice and listen all the songs in our library',
    label: 'Our songs',
  },
  // {
  //   color: '#000C00',
  ...
  setup(props) {
    ...
    const setupEventListeners = () => {
      ...
      const handleMouseLeave = () => {
        ...
        ;(props.gridRef?.querySelectorAll('.card') as NodeListOf<HTMLDivElement>).forEach(
          (card) => {
            card.style.setProperty('--glow-intensity', '0')
          },
        )
...
.card {
  /* Elimina overflow-hidden o cámbialo a visible */
  overflow: visible !important;
  /* Asegura que no limite el contenido */
  min-height: auto !important;
}
