...
const setup = () => {
  ...
  try {
    ...
    // Estilos FIXED para que siga el cursor correctamente
    ...
    canvas.style.pointerEvents = 'none' // Importante: no interceptar eventos del mouse

    // Limpia cualquier canvas previo
...
`
    ...
    const resize = () => {
      ...
      if (props.blindMinWidth && props.blindMinWidth > 0) {
        ...
        const effective = props.blindCount
          ? Math.min(props.blindCount, maxByMinWidth)
          : maxByMinWidth
    ...
    // Event listener para el movimiento del mouse en toda la ventana
    ...
    // Escuchar en toda la ventana, no solo en el canvas
